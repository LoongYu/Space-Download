...
- 取消 0.9.2 的双列卡片方案，全局设置与站点设置恢复为从上到下的单列阅读顺序
- 设置窗口调整为 960×840，每个设置区块完整占据右侧宽度，控件统一靠右对齐
- 继续保持无 `ScrollView` 单屏显示，并覆盖 YouTube 字幕语言和 Cookies 同时展开的最高内容状态

## 0.9.2 (2026-08-31)

### 设置页单屏布局与仅标题模板

- 原生 macOS 设置窗口调整为 960×680，彻底移除设置内容 `ScrollView`，全局设置改为双栏卡片布局
- 站点设置改为顶部紧凑站点选择器、下方“文件与媒体 / 站点采集”双栏布局，最复杂的 YouTube 字幕与 Cookies 同时展开状态也可单屏显示
- 所有站点的文件命名模板新增首项“仅标题”，规则为 `%(title)s`；原有“仅标题(id)”及其它带 ID 模板保持兼容
- 新增全部设置页面不存在 `NSScrollView` 的界面回归测试，并验证“仅标题”在七个站点间独立保存
...
- 修复短进程退出时输出读取与终止回调竞争、可能丢失最后一段 yt-dlp 日志的问题，并增加 25 次快速进程回归
