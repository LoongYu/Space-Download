...
- 🔓 YouTube 默认无需认证；受人机验证或登录限制时可读取本机 Chrome 会话，也可单独选择 Cookies 文件
...
`native/` 目录包含独立的 SwiftUI 重构工程。原生版已接入真实 `yt-dlp` 下载引擎，并完成设置持久化、目录选择、多链接、Pornhub 网页分页、YouTube 单视频/播放列表/频道、实时进度、失败统计、翻译、封面、代理和认证；现有 Python 客户端保持不变。
